...
The GLM approach is purely analytical, while the NECDF approach relies on 
pilot samples. These pilot samples are used to estimate $p = P(X < Y)$, 
where $X$ and $Y$ are independent random variables, representing the
control- and experimental group. With the estimate of $p$, sample size
estimation proceeds analytically as detailed by Noether (1987).
...
All function are documented, so that you can use R's builtin help system. You can also refer to the [online documentation](https://jobrachem.github.io/skewsamp/), which includes a [list of all functions](https://jobrachem.github.io/skewsamp/reference/index.html).
